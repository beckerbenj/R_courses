...
\texttt{?exists} returns a logical if a reference if found in an environment. The search jumps to the enclosing environment until the object is found when \texttt{inherits = TRUE} (the default).
...
\begin{frame}{Where does R find stuff?}
Unexported functions from packages one binding environment: the \texttt{namespace:<name>} environment 
...
\begin{itemize}
  ...
  \item a \emph{method} is specific implementation of procedure, and is associated to classes.
  \item \emph{inheritance}: classes are organized in hierarchy. ``is-a-type-of''-relation.
  \item \emph{method-dispatch}: the process of finding the correct method to apply for a specific object.
\end{itemize}
\end{frame}
...
\begin{itemize}
  \item methods belong \emph{generics} (or generic functions)
