...
\textbf{You ...}
\begin{itemize}
  \item are a frequent R user (e.g. for data wrangling/analyses)
  \item have a rough understanding of R as a programming language
  \item have already written some R functions yourself
...
Higher order functions are functions that either take \textbf{functions as input} or return \textbf{functions as output}.
\end{frame}
...
\begin{frame}{\texttt{apply}-family}
A popular alternative from the tidy-verse: \texttt{purrr}
\begin{itemize}
  \item \texttt{map} vector / list $\rightarrow$ list
  \item \texttt{map2} multiple vectors / lists $\rightarrow$ vector (matrix)
  \item ...
\end{itemize}
\end{frame}
...
\begin{itemize}
  ...
  \item Own discrepancies between different machines
...
\begin{itemize}
  ...
  \item Careful: RStudion GUI has limited support for this
\end{itemize}
\end{frame}
...
\begin{itemize}
  ...
  \item It makes you code easier accessible (for others and yourself)
  \item It gives a great framework for documenting your code 
  \item There are great tools for testing your code
...
\begin{itemize}
  \item Create a Github Repository and clone it locally
  \item Create an R project (-> R package) in the existing repository
  \item Use \texttt{usethis} if you want to create more specific architecture
\end{itemize}
\end{frame}

\begin{frame}{R functions}
Your actual code lives in the \texttt{R} folder. Organize your functions logically and avoid very long scripts.
\end{frame}
...
\begin{itemize}
  \item Good test coverage serves is a quality attribute of a good and stable package
...
Writing R packages
\begin{itemize}
\item Extending R (\url{https://r-bio.github.io/intro-git-rstudio/})
