...
\begin{frame}{Motivation}
	...
	\begin{itemize}
			\item Past Self
			\begin{itemize}
			  \item Is the biggest mess in existance
...
\begin{center}
  \begin{tabular}{| l | l |}
    ...
    matrix      & an vector with \texttt{"dim"} argument: two dimensions \\
...
A list is a ``vectors'' that can contain any type of element

\begin{itemize}
  \item the elements can differ $\leftrightarrow$ attomic vectors
