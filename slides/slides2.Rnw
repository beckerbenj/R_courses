...
Try to limit your \emph{package-dependencies}. Only load (i.e., \texttt{library()}) the packages that you absolutely need. If you are only using \texttt{dplyr}, it does not make sense to load the complete \texttt{tidyverse}.
...
Forget about \texttt{attach()}! Don't use it, unless you completely understand what happens (see \texttt{?attach}).
...
\textbf{Every project should get an RStudio Project!}
%Don't use \texttt{setwd(\"path\/to\/my\/local_folder\")}
Issues when
\begin{itemize}
...
@
\texttt{test} should be either \texttt{TRUE} or \texttt{FALSE} (or code that results one of both). If \texttt{test == TRUE}, than \texttt{some\_computations} is executed, if \texttt{test == FALSE}, than not.
