...
\begin{frame}[fragile]{Conditional Computation - if}
The \emph{test} should have length 0.
...
\end{itemize}
<<next, echo=TRUE, eval = FALSE, results="markup">>=
index <- 0
repeat {
  index <- index + 1
...
\begin{itemize}
  \item Use \texttt{seq}, \texttt{seq\_len}, or \texttt{seq\_along}
\end{itemize}
<<seq, echo=TRUE, eval = TRUE, results="markup">>=
x <- numeric()
...
A functional is a function that takes another funciton as an argument. \\
...
<<lapply, echo=TRUE, eval = TRUE, results="markup">>=
...
\end{itemize}
<<lapply2, echo=TRUE, eval = TRUE, results="markup">>=
...
\end{itemize}
<<apply, echo=TRUE, eval = TRUE, results="markup">>=
...
\begin{frame}[fragile]{S3 - class}
<<class2, echo=TRUE, results='markup'>>=
class(iris)
head(iris)
@
\end{frame}
...
\begin{frame}[fragile]{S3 - class}
<<class3, echo=TRUE, results='markup'>>=
...
\begin{frame}[fragile]{S3 - class}
Good practice: \emph{constuctor function}
\begin{itemize}
...
<<method, echo=TRUE, results='markup'>>=
...
<<method2, echo=TRUE, results='markup'>>=
center <- function(x, ...)
  UseMethod("center")
...
pisa <- readRDS("C:/R_courses/Programming/R_courses/data/pisaPlus_CF.RDS")
@
Objects can have more than one classes
<<inheritance1, echo=TRUE, results='markup', size = "small">>=
