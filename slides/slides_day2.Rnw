...
\subsection{Non-standard Output}
...
\begin{itemize}
  ...
  \item Additional: Console output
  \item Additional: Output to graphic device
\end{itemize}
\end{frame}
...
\begin{frame}[fragile]{Output}
Graphics output: Standard plots, ggplot2
<<echo=FALSE, results='asis', message=TRUE>>=
add_things_plot <- function(x = 1) {
  x2 <- x*2
  out <- x + x2
  plot(out)
  NULL
...
\begin{itemize}
  \item Avoid output to the console
  \item Choose carefully if something warrants a message, warning or error
  \item Plotting functions should return nothing or the plot as an object
\end{itemize}
\end{block}
\end{frame}
