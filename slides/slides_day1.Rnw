...
\begin{itemize}
  ...
  \item Scoping
  \item Output
  \item Conditional Computation - if
  \item Work flow
\end{itemize}
\end{frame}
...
\subsection{Conditional Computation - if}
...
\begin{block}{Programming advice}
\begin{itemize}
  \item If possible, avoid using else()
  \item Use early return(), stop(), etc. instead
\end{itemize}
\end{block}
\end{frame}
