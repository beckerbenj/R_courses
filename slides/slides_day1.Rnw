...
\begin{frame}[fragile]{Conditional Computation - if}
Solution using meaningful initialisation
...
Most functions take one or multiple inputs. These are usually:
\begin{itemize}
