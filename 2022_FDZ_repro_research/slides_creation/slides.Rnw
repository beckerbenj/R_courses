...
\textbf{knitr} executes each code chunk sequentially \textbf{in a new R-session}, transforms the results into markdown formatted text that replace the code chunks.

\textbf{pandoc} converts the Markdown text (including the results of the code) into the chosen output format (like html).
