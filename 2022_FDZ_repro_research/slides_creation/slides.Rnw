...
Peikert, A.,van Lissa, J., Brandmaier, A.M. (2021). Reproducible Research in R: A Tutorial on How to Do the Same Thing More Than Once. \emph{Psych 2021, }3, 836–867.
\end{frame}
...
Name		& Value 		& Related to 				& Description \\
\hline
\texttt{echo} 		& \texttt{TRUE} 		& results 			& If FALSE, knitr will not ``echo'' the code in the chunk above it’s results in the final document. \\
\hline
...
via a Personal access token for HTTPS
\begin{itemize}
...
\item Use %gitcreds::gitcreds_set()
\end{itemize}
\end{frame}
...
R Resources
\begin{itemize}
\item Avanced R Ed. 1 (\MYhref{http://adv-r.had.co.nz/})
\item Avanced R Ed. 2 (\MYhref{https://adv-r.hadley.nz/})
\item R Inferno (\MYhref{https://www.burns-stat.com/pages/Tutor/R_inferno.pdf})
\item R Packages (\MYhref{https://r-pkgs.org/})
\item Clean Code (\MYhref{https://mooc.aptikom.or.id/pluginfile.php/1174/mod_resource/content/1/Clean\%20Code_\%20A\%20Handbook\%20of\%20Agile\%20Software\%20C\%20-\%20Robert\%20C.\%20Martin.pdf})
\end{itemize}

Reproducible Research
\begin{itemize}
\item CRAN Tastk View (\MYhref{https://cran.r-project.org/web/views/ReproducibleResearch.html})
\end{itemize}
