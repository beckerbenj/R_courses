...
roundDataFrame2 <- function (df, roundN = 3) {
  for(i in seq_along(df)) {
    if(is.numeric(df[, i])) df[, i] <- round(df[, i], digits = roundN) 
  }
  df
}

roundDataFrame2(mtcars, roundN = 1)
roundDataFrame2(iris, roundN = 1)
...
prop_table_by_all2 <- function (df, dep, by_var, useNA = "no", round_perc = 1) {
  stopifnot(is.data.frame(df))
  stopifnot(is.character(dep) && length(dep) == 1)
  stopifnot(is.character(by_var) && length(by_var) == 1)
  ...
  gesamt <- prop_table(df[, dep], useNA = useNA)
  out_list <- by(df, INDICES = df[, by_var], function(df) prop_table(df[, dep], useNA = useNA))
  out <- do.call(plyr::rbind.fill, out_list)
  rownames(out)[nrow(out)] <- "Total"
  out
}

prop_table_by_all2(pisa, dep = "books", by_var = "migration")
prop_table_by_all2(pisa, dep = "grade_bio_t1", by_var = "books")
