...
This is less useful for writing functions, but can be handy if you extract other types of data via LMMs. 
...
# read as json
data1 <- jsonlite::fromJSON(data, flatten = TRUE)
head(data1)
```
...
```{r}
data1[data1$country == "Germany",]
data1[data1$country == "Ukraine",]
```
...
If you do this a second time, do you get the same resutls?

```{r}
llm_data2 <- chat_google_gemini(
  system_prompt = "you are an reliable scientist. You are honest and terse."
)
...
# read as json
data2 <- jsonlite::fromJSON(data, flatten = TRUE)
head(data2)
```



Note that the computations below will differ everytime the code is ran.


```{r}
countries_in_both <- data1$country[data1$country %in% data2$country]

# which countries are not in the first option
data1[!data1$country %in% countries_in_both, ]


# do the numbers correspond?
combined <- merge(data1[, c(1, 3)], data2[, c(1, 3)], by = "country")
combined[(combined$population.y-combined$population.x) != 0,]
```
