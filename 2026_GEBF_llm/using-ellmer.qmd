...
To use `ellmer`, you need to have access to a GenAI model trough an API. Hence, you have to set up an account with a provider (e.g., OpenAI, Anthropic, etc.) and get an API key. Once you have the API key, you can set it as an environment variable in your R session.
...
The following example/workflow is less useful for writing functions, but can be handy if you extract other types of data via GenAI. 

In this example, an additional prompt is used in a separate markdown file `my_promt.md`. You can find this file [here](https://users.ugent.be/~ddebeer/workshops/GEBF/ellmer-example/my_promt.md).
...
# read as json
countries1 <- jsonlite::fromJSON(data, flatten = TRUE)
head(countries1)
```

```{r}
#| echo: false
countries1 <- readRDS("countries1.RDS")
head(countries1)
```


Lets compare the results with wikipedia for 

* [Germany](https://en.wikipedia.org/wiki/Germany)
* [Ukraine](https://en.wikipedia.org/wiki/Ukraine)

```{r}
countries1[countries1$country == "Germany",]
countries1[countries1$country == "Ukraine",]
```
...
If you do this a second time, do you get the same results?

```{r}
#| eval: false
llm_countries2 <- chat_google_gemini(
  system_prompt = "you are an reliable scientist. You are honest and terse."
)

data <- llm_countries2$chat(
  paste(collapse = "\n", readLines("2026_GEBF_llm/my_promt.md")),
  echo = "none"
)
...
# read as json
countries2 <- jsonlite::fromJSON(data, flatten = TRUE)
head(countries2)
```

```{r}
#| echo: false
countries2 <- readRDS("countries2.RDS")
head(countries2)
```



Note that the computations below will differ every time the code is ran.

When rendering, there were some differences between the first and second time the data was asked.


```{r}
#| tbl-cap: "Countries Included in the Second Querry, but not in the First"
countries_in_both <- countries1$country[countries1$country %in% countries2$country]

# which countries are not in the first option
knitr::kable(countries1[!countries1$country %in% countries_in_both, ])
```

```{r}
#| tbl-cap: "Countries Included in the First Querry, but not in the Second"
countries_in_both <- countries1$country[countries2$country %in% countries1$country]

# which countries are not in the first option
knitr::kable(countries2[!countries1$country %in% countries_in_both, ])
```

Just spelling issues, it seems.

Lets check if the numbers correspond.

```{r}
#| tbl-cap: First Six Countries with Different Population Numbers
# do the numbers correspond?
combined <- merge(countries1[, c(1, 3)], countries2[, c(1, 3)], by = "country")

knitr::kable(head(combined[(combined$population.y-combined$population.x) != 0,]))
```

For many countries the population number differs. Similar, but not reproducible. 





Whether or not this is a good idea depends on your use case. Be aware that GenAI can make mistakes and confabulate.
