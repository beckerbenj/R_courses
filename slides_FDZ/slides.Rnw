...
\begin{frame}[fragile]{Loops \& Iteration - nested loop}
...
\begin{frame}{Loops \& Iteration - apply}
...
<<apply-def, echo=TRUE, results="markup", size="footnotesize", eval = FALSE>>=
apply(matrix, 
      MARGIN = 1,       # 1 = iterate over rows; 2 = iterate over columns)
