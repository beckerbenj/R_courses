...
\begin{itemize}
  ...
  \item \emph{Functionals} \& split-apply-combine
...
\begin{frame}[fragile]{Conditional Computation - if}
The \textbf{\texttt{test}} should have length 1!
...
The most commonly used tools are: 
\begin{itemize}
  \item loops (\texttt{for}, \texttt{while}, \texttt{repeat})
  \item \emph{functionals }(\texttt{apply} - family)
\end{itemize}
\end{frame}
...
\begin{frame}{Loops \& Iteration - \emph{Functionals}}
There are specific functions that allow efficient and clean iteration. \\
We focus on the \texttt{apply}-family. These functions \emph{apply} a function repeatedly. \\
...
\end{itemize}
(also see the \href{https://purrr.tidyverse.org/}{\texttt{purrr}} package)
\end{frame}
...
means <- lapply(mtcars[1:7], FUN = mean)
str(means)
@
\end{frame}
...
\begin{frame}{\emph{Functionals}}
More on how these \emph{functionals} can be used tomorrow.
\end{frame}
...
<<echo=TRUE, eval=TRUE>>=
return_early <- function(x, early = FALSE) {
  x2 <- x*2
  if(early) (return(x2))
  out <- x + x2 # not executed
  out
...
\begin{frame}{\texttt{apply}-family}
A popular alternative from the tidyverse: \href{https://purrr.tidyverse.org/}{\texttt{purrr}} 
\begin{itemize}
